...
syscall_handler (struct intr_frame *f UNUSED)
{
  ...
  switch (args[0]){
    ...
    case SYS_SEEK:
      ...
      for (seek_e = list_begin (&thread_current()->files); seek_e != list_end (&thread_current()->files); seek_e = list_next (seek_e)) { 
    ...
    case SYS_TELL: 
      ...
      for (tell_e = list_begin (&thread_current()->files); tell_e != list_end (&thread_current()->files); tell_e = list_next (tell_e)){         
...
void check_valid_ptr(const uint8_t *addr, int range, struct intr_frame *f UNUSED){
  if (!is_user_vaddr(addr) || !is_user_vaddr(addr+range) || !pagedir_get_page(thread_current()->pagedir, addr)){
    syscall_exit(-1, f);
  }
}
